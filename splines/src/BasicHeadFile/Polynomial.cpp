...
{
    std::vector<double> res;
    res.push_back(0); // 积分常数默认为0
    for (int i = 0; i <= this->deg(); ++i) {
        res.push_back(coefs[i] / static_cast<double>(i+1));
    }
    return Polynomial(res);
}

Polynomial Polynomial::Integrate(int degree) const
{
    if (degree < 0)
        throw std::invalid_argument("求积分次数必须大于等于0！");
    Polynomial res(*this);
    for (int i = 0; i < degree; ++i) {
        res = res.Integrate();
    }
...
const Polynomial Polynomial::reformulate(double x0) const
{
    std::vector<double> res;
    for (int i = 0; i <= deg(); i++)
    {
        res.push_back(this->diff(i, x0) / __factorial__(i));
    }
    return Polynomial(res);
}
