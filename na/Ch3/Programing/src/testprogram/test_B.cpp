...
int main()
{
    ...
    // std::vector<double> x = {27.7, 28.0, 29.0,30.0};
    // std::vector<double> y = {4.1, 4.3, 4.1, 4.1};
    // 设置插值条件
    ...
    // 构造三次样条
    Spline<3, SplineType::B_spline> spline(cond); //！！！！！不同阶数的样条，这里需要修改！！！！！

    //输出输入条件
    std::ofstream outfile1("spline_data_B_input.txt");
    if (!outfile1)
    {
    ...
    spline.print_Polys();
    // 打开文件用于写入样条插值结果
    std::ofstream outfile("spline_data_B.txt");
    if (!outfile)
    {
        std::cerr << "无法打开输出文件。" << std::endl;
        return 1;
    ...
    std::cout << "样条插值结果已写入 spline_data_B.txt 文件。" << std::endl;
