...
x_spline = []
y_spline = []
with open('spline_data_B.txt', 'r') as f:
    for line in f:
...
# 读取文件内容
with open('spline_data_B_input.txt', 'r') as f:
    lines = f.readlines()
    # 跳过描述性文本，只保留实际数据行
...
plt.plot(x_spline, y_spline, label='B Spline', color='blue')
