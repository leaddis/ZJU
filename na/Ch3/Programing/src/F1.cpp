...
int main() {
    ...
    // 图1：原始截断函数 (t - 1)_+
    output_data(f0, -0.5, 2.5, 0.01, "fig1_0_original.csv");

    // 基础截断函数 f(t) = (t - t_i)_+ 对于 i=1 则 xi=1.0
    auto f1 = [&](double t){return truncated_linear(t, t_i);};

    // 图1：原始截断函数 (t - 1)_+
    output_data(f1, -0.5, 2.5, 0.01, "fig1_1_original.csv");

    // 基础截断函数 f(t) = (t - t_i)_+ 对于 i=1 则 xi=1.0
    auto f2 = [&](double t){return truncated_linear(t, t_ip1);};

    // 图1：原始截断函数 (t - 1)_+
    output_data(f2, -0.5, 2.5, 0.01, "fig1_2_original.csv");
    ...
    output_data(line_iip1, -0.5, 2.5, 0.01, "fig2_line_iip1.csv");
    ...
    output_data(line_im1i, -0.5, 2.5, 0.01, "fig3_line_im1i.csv");
    ...
    output_data(b_spline_n1, -0.5, 2.5, 0.01, "fig4_bspline.csv");
    ...
    cout << "Data generated. You can now use Python (matplotlib) to plot the CSV files:\n";
    cout << "fig1_original.csv, fig2_line_iip1.csv, fig3_line_im1i.csv, fig4_bspline.csv\n";
    cout << "Adjust/extend similarly to produce all 6 intermediate figures in Figure 3.2.\n";
    return 0;
}
