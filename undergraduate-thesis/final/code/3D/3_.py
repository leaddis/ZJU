...
# 1. 物理模型与参数设置
# ==========================================
nx, ny = 40, 40                 # 网格40x40（原60x60内存过大，现自由度1600）
N_dofs = nx * ny
h = 1.0 / (nx + 1)
n_snapshots_total = 50000       # 总快照数（原1000的50倍）
rank_truncation = 20            # 固定截断阶数

# 生成更密集的评估节点（对数间隔，从100到总快照数，约20个点）
...
print(f"网格: {nx}x{ny}, 自由度: {N_dofs}, 截断秩 r={rank_truncation}")
...
mu3_samples = np.random.uniform(-0.2, 0.2, n_snapshots_total)

# 使用 float32 节省内存（若精度足够）
...
def incremental_svd_tracking(U, snapshot_nodes, r, block_size=100):
    ...
    for start in range(0, U.shape[1], block_size):
        ...
        while node_idx < len(snapshot_nodes) and processed >= next_node:
            ...
            node_Qs.append(Q.copy())   # 保存基副本用于后续误差计算
            node_idx += 1
...
plt.tight_layout()
plt.savefig('svd_vs_snapshots.png', dpi=300, bbox_inches='tight')
print("-> 曲线图已保存为 'svd_vs_snapshots.png'")
