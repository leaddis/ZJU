...
def calculate_markov_factor(data, eligible_stocks, cross_section_date, window=21, thresholds=[-0.01, 0, 0.01]):
    ...
    # 初始化马尔可夫转移因子
    markov_factor = pd.Series(index=eligible_stocks, dtype=float)
    valid_stocks = []
    ...
    for stock in eligible_stocks:
        if stock in returns.index:
            ...
            # 检查是否包含所有状态
            if len(stock_states.value_counts()) >= 2:  # 至少需要两个状态才能计算转移概率
                # 计算转移概率矩阵
                ...
                # 提取转移概率
                try:
                    P00 = transition_matrix.loc[0, 0]  # 大幅下跌后继续大幅下跌的概率
                except KeyError:
                    P00 = 0  # 如果状态不存在，设置默认值

                try:
                    P11 = transition_matrix.loc[1, 1]  # 小幅下跌后继续小幅下跌的概率
                except KeyError:
                    P11 = 0  # 如果状态不存在，设置默认值

                try:
                    P22 = transition_matrix.loc[2, 2]  # 小幅上涨后继续小幅上涨的概率
                except KeyError:
                    P22 = 0  # 如果状态不存在，设置默认值

                try:
                    P33 = transition_matrix.loc[3, 3]  # 大幅上涨后继续大幅上涨的概率
                except KeyError:
                    P33 = 0  # 如果状态不存在，设置默认值

                # 构建转移概率因子
                markov_factor[stock] = (P33 + P22) - (P11 + P00)  # 上涨趋势 - 下跌趋势
            ...
            print(f"Stock {stock} is not in returns index and will be removed from eligible_stocks.")         
...
def visualize_results(group_returns, hedge_return, next_returns, groups, cross_section_date):
    ...
    os.makedirs("Bar plot 4", exist_ok=True)
    os.makedirs("Box plot 4", exist_ok=True)
...
if __name__ == "__main__":
    ...
    HEDGE_RETURN_FILE = "hedge_return_4.csv"
